...
const MovieDetails = () => {
    ...
    return (
        <>
            <Button className='m-3 mb-3'
                onClick={() => navigate(-1 || '/')}
            >back</Button>
            <div className="cont">
                <div className="img-container">
                    <img src={data?.data?.image?.medium} alt={data?.data?.name} className="img" />
                </div>
                ...
                >
                    ...
                    <Modal.Body className="d-flex gap-3 align-items-start w-100 flex-wrap">
                        <div className="">
                            <img src={data?.data?.image?.medium} alt={data?.data?.name} className="" />
                        </div>
                        <div className="d-flex justify-content-center flex-column fs-5">
                            ...
                            <div className="mt-3 d-flex w-100 flex-column gap-2">
                                <input type="text" name="name" value={input.name} placeholder="Your Name" onChange={handleInput}
                                    className="p-1"
                                />
                                <input type="tel" name="phone" value={input.phone} placeholder="Phone Number" onChange={handleInput}
                                    className="p-1"
                                />
                            </div>
                        </div>
